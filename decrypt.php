...
class QuantumWarpDriveDecryption {
    private $warp;
    private $c = 299792458;
    ...
    public function decrypt($encryptedPackage, $x, $t, $hmacPin) {
        ...
        $data = base64_decode($encryptedPackage);
        $iv = substr($data, 0, 16);
        $ciphertext = substr($data, 16, -32);
        $hmac = substr($data, -32);
        
        if (!hash_equals(hash_hmac('sha256', $ciphertext, $hmacPin, true), $hmac)) {
            throw new Exception('HMAC verification failed. The message may have been tampered with.');
        }
        
        $decrypted = openssl_decrypt($ciphertext, 'AES-256-CBC', $key, OPENSSL_RAW_DATA, $iv);
        
        if ($decrypted === false) {
            throw new Exception('Decryption failed. The message may be corrupted.');
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['x']) || !isset($_POST['t']) || !isset($_POST['encryptedPackage']) || !isset($_POST['hmacPin'])) {
    ...
    $x = floatval($_POST['x']);
    $t = floatval($_POST['t']);
    $encryptedPackage = $_POST['encryptedPackage'];
    $hmacPin = $_POST['hmacPin'];

    try {
        $qwde = new QuantumWarpDriveDecryption();
        $result = $qwde->decrypt($encryptedPackage, $x, $t, $hmacPin);

        echo "Decrypted Message: " . $result['decryptedMessage'] . "\n\n";
